...
import quarot.fake_quant.quant_utils as quarot_quant_utils
...
def omniquant(
    ...
    quarot_quant_utils.add_actquant(model) #Add Activation Wrapper to the model
    logger.info("\n==========Model architecture after rotate & activation wrapper==========")
    logger.info(f"\n{model}")
    hlayers = quarot_quant_utils.find_qlayers(model)
    for name in hlayers:
        if 'down_proj' in name:
            had_K, K = hadamard_utils.get_hadK(model.config.intermediate_size)
            hlayers[name].online_full_had = True
            hlayers[name].had_K = had_K
            hlayers[name].K = K
            hlayers[name].fp32_had = False #args.fp32_had
        if 'o_proj' in name:
            had_K, K = hadamard_utils.get_hadK(model.config.num_attention_heads)
            hlayers[name].online_partial_had = True
            hlayers[name].had_K = had_K
            hlayers[name].K = K
            hlayers[name].had_dim = model.config.hidden_size//model.config.num_attention_heads
            hlayers[name].fp32_had = False #args.fp32_had
    #############################
    ...
    for i in range(len(layers)):
        ...
        if args.let:
            ...
            qlayer.register_parameter("qkt_smooth_scale",torch.nn.Parameter(torch.ones(layer.self_attn.q_proj.module.out_features,device=dev, dtype=dtype))) # 
        ...
        if args.epochs > 0:
            ...
            loss_scaler = omniutils.NativeScalerWithGradNormCount()
