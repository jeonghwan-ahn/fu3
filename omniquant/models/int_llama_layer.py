...
from quarot.fake_quant.rotation_utils import *
...
class QuantLlamaMLP(nn.Module):
    def __init__(
        ...
        # [Fuquant] Wrap the QuantLinear with an ActQuantWrapper
        self.gate_proj = ActQuantWrapper(QuantLinear(unwrap_module(org_module.gate_proj),
                                           args.weight_quant_params,
                                           args.act_quant_params))
        self.down_proj = ActQuantWrapper(QuantLinear(unwrap_module(org_module.down_proj),
                                           args.weight_quant_params,
                                           args.act_quant_params))
        self.up_proj = ActQuantWrapper(QuantLinear(unwrap_module(org_module.up_proj),
                                           args.weight_quant_params,
                                           args.act_quant_params))
...
class QuantLlamaAttention(nn.Module):
    ...
    def __init__(self, 
        ...
        # [Fuquant] Wrap the QuantLinear with an ActQuantWrapper
        self.k_proj = ActQuantWrapper(QuantLinear(
            unwrap_module(org_module.k_proj),
            args.weight_quant_params,
            args.act_quant_params,
        ))
        self.v_proj = ActQuantWrapper(QuantLinear(
            unwrap_module(org_module.v_proj),
            args.weight_quant_params,
            args.act_quant_params,
        ))
        self.q_proj = ActQuantWrapper(QuantLinear(
            unwrap_module(org_module.q_proj),
            args.weight_quant_params,
            args.act_quant_params,
        ))
        self.o_proj = ActQuantWrapper(QuantLinear(
            unwrap_module(org_module.o_proj),
            args.weight_quant_params,
            args.act_quant_params,
        ))
...
class QuantLlamaDecoderLayer(nn.Module):
    ...
    def smooth_and_quant_temporary(self):
        if self.let:
            ...
            smooth_ln_fcs_temporary(self.input_layernorm,[self.self_attn.q_proj.module, self.self_attn.k_proj.module, self.self_attn.v_proj.module],
                                    self.qkv_smooth_scale,self.qkv_smooth_shift)
            smooth_ln_fcs_temporary(self.post_attention_layernorm,[self.mlp.up_proj.module,self.mlp.gate_proj.module],
                                    self.fc1_smooth_scale,self.fc1_smooth_shift)
            smooth_fc_fc_temporary(self.self_attn.v_proj.module,self.self_attn.o_proj.module,
                                self.out_smooth_scale, self.out_smooth_shift)
            smooth_q_k_temporary(self.self_attn.q_proj.module, self.self_attn.k_proj.module,
                                self.qkt_smooth_scale)
            self.mlp.down_proj.module.temp_weight = self.mlp.down_proj.module.weight
        else:
        ...
        for name, module in self.named_modules():
            if isinstance(module, QuantLinear):
                print("There exists QuantLinear")
